...
export default function SignalsPage() {
  ...
  const runScan = async () => {
    ...
    try {
      if (isSandbox) {
        // Return simulated scan results matching active assets
        const mockScan = [
          { ticker: "AAPL", signal: "BUY", confidence: 87.0 },
          { ticker: "NVDA", signal: "BUY", confidence: 91.0 },
          { ticker: "TSLA", signal: "SELL", confidence: 64.0 },
          { ticker: "MSFT", signal: "BUY", confidence: 78.0 },
          { ticker: "GOOGL", signal: "HOLD", confidence: 72.0 }
        ];
  ...
  const runPrediction = async () => {
    ...
    if (isSandbox) {
      // Local progress loader simulation
      const steps = [
        { progress: 20, status: "Initializing Neural Networks..." },
        { progress: 45, status: "Analyzing News Sentiment..." },
        { progress: 70, status: "Running LSTM Price Forecasting..." },
        { progress: 90, status: "Computing Ensemble Signal..." },
      ...
      const interval = setInterval(() => {
        if (stepIdx < steps.length) {
          ...
          if (step.status === "complete") {
            ...
            const currentPrice = ticker === "NVDA" ? 900.0 : ticker === "TSLA" ? 170.0 : ticker === "MSFT" ? 420.0 : 180.0;
            const signalType = ticker === "TSLA" ? "SELL" : ticker === "GOOGL" ? "HOLD" : "BUY";
            const confidence = ticker === "TSLA" ? 64.0 : ticker === "GOOGL" ? 72.0 : 87.0;
            const trendText = signalType === "BUY" ? "BULLISH" : signalType === "SELL" ? "BEARISH" : "NEUTRAL";
            ...
            const mockResult = {
              ...
              risk_assessment: {
                volatility: ticker === "TSLA" ? "high" : "moderate",
                suggested_position_size_pct: ticker === "TSLA" ? 5.0 : 12.5,
                suggested_stop_loss_pct: ticker === "TSLA" ? 8.5 : 4.5,
                risk_reward_ratio: 2.45,
                reward_target_pct: ticker === "TSLA" ? 12.0 : 10.8,
              },
              signal_breakdown: {
                lstm: {
                  predicted_price: signalType === "BUY" ? currentPrice * 1.054 : signalType === "SELL" ? currentPrice * 0.946 : currentPrice,
                  expected_return: signalType === "BUY" ? 0.054 : signalType === "SELL" ? -0.054 : 0.0,
                },
                xgboost: {
                  trend: trendText,
                ...
                sentiment: {
                  score: signalType === "BUY" ? 0.68 : signalType === "SELL" ? -0.52 : 0.12,
                  recent_headlines: [
                    { headline: `${ticker || "Asset"} releases generative neural network engine`, sentiment: signalType === "BUY" ? "positive" : "neutral" },
                    { headline: `Retail buyers increase options exposure on ${ticker || "Asset"}`, sentiment: signalType === "BUY" ? "positive" : "neutral" },
                    { headline: `Technical signals suggest trend inflection for ${ticker || "Asset"}`, sentiment: signalType === "BUY" ? "positive" : "neutral" }
                  ]
                },
                model_scores: {
                  lstm_expected_return_pct: signalType === "BUY" ? 5.4 : signalType === "SELL" ? -5.4 : 0.0,
                  xgboost_confidence_pct: confidence,
                  sentiment_score: signalType === "BUY" ? 0.68 : signalType === "SELL" ? -0.52 : 0.12,
                }
              }
            };
            ...
            toast({
              ...
              description: `AI Ensemble output for ${ticker || "Asset"} is ready.`
            });
          }
          stepIdx++;
        }
    ...
    try {
      ...
      if (response.status === 401 || response.status === 403) {
        // Try once with refreshed headers
        ...
        if (!retryRes.ok) {
          throw new Error(`Authentication failed (${retryRes.status}). Please sign out and sign in again.`);
        }
  ...
  const getSignalColor = (signal: string) => {
    ...
    return "text-text-muted border-border bg-card/50";
  };
      ...
      try {
        if (isSandbox) {
          ...
          for (let i = 20; i >= 1; i--) {
            ...
            history.push({
              label: formattedDate,
              price: currentPrice - (i * (currentPrice * 0.005)) + variance,
              isPrediction: false
            });
          }
  ...
  const SentimentRow = ({ label, value }: { label: string, value: number }) => {
    ...
    return (
      <div className="flex flex-col gap-1.5 font-mono text-[11px] w-full">
        ...
        <div className="h-3 w-full bg-card/50 rounded-full overflow-hidden flex relative border border-border/30">
  ...
  return (
    <div className="flex flex-col gap-6 max-w-7xl mx-auto w-full pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4 py-2 w-full">
                {[1, 2, 3].map(i => (
                  <div key={i} className="flex items-center justify-between animate-pulse">
                    ...
                    <div className="h-3.5 bg-border rounded w-8" />
                  </div>
                ))}
              </div>
            ) : scanError ? (
        ...
        <CardSurface className="lg:col-span-2 p-6 flex flex-col justify-center gap-6">
          ...
          {loading ? (
            <div className="space-y-4">
              <div className="space-y-3.5 py-2 border-l-2 border-border/20 ml-2.5 pl-6 relative">
                <PipelineStep 
            ...
            <div className="flex items-center justify-center h-32 border border-dashed border-border rounded-xl">
               <span className="text-sm text-text-muted">Configure parameters and click &quot;RUN AI PIPELINE&quot; to begin</span>
            </div>
          ) : null}
      ...
      <div className="min-h-[600px] flex flex-col gap-6">
        {loading ? (
          <div className="flex flex-col gap-6">
            <CardSurface className="h-[200px] animate-pulse bg-muted/50" />
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-6">
            {/* HERO SIGNAL CARD */}
            <CardSurface className="p-8 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
               ...
               <div className="flex items-center gap-8">
                 ...
                 <div>
                    <h2 className="text-sm font-mono text-text-muted uppercase tracking-[0.2em] mb-1">Ensemble Decision</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <CardSurface className="p-6 flex flex-col gap-4">
                      ...
                      <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={200}>
                        <LineChart data={historyData}>
                          ...
                          <Tooltip 
                           ...
                           />
                          <Line type="monotone" dataKey="price" stroke="var(--text-muted)" strokeWidth={2} dot={false} />
                          <Line type="monotone" dataKey="price" data={historyData.filter(d => d.isPrediction)} stroke="var(--accent-teal)" strokeWidth={2} strokeDasharray="5 5" dot={false} activeDot={{ r: 6, fill: '#fff', stroke: 'var(--accent-teal)', strokeWidth: 2 }} />
                          {todayLabel && (
                            <ReferenceLine 
                              x={todayLabel} 
                ...
                <div className="p-3 rounded-lg bg-card/50 border border-border mt-10">
                 <p className="text-xs text-text-muted leading-relaxed italic">
                   Forecast Price: <strong className="text-text-primary">${(result.signal_breakdown?.lstm?.predicted_price || 0).toFixed(2)}</strong>
              ...
              <CardSurface className="p-6 flex flex-col gap-4">
                        ...
                        <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={200}>
                          <BarChart data={[
                            ...
                            <Bar dataKey="val" radius={[4, 4, 0, 0]} barSize={40} fillOpacity={0.85}>
                              ...
                              <LabelList dataKey="val" position="top" formatter={(v: any) => `${Number(v).toFixed(1)}%`} style={{ fontSize: 10, fill: "var(--text-primary)", fontWeight: "bold" }} />
                            </Bar>
                          </BarChart>
                        </ResponsiveContainer>
                      </ChartWrapper>
                ...
                <div className="p-3 rounded-lg bg-card/50 border border-border mt-10">
                  <p className="text-xs text-text-muted leading-relaxed italic">
                    Trend Prediction: <strong className="text-text-primary">{result.signal_breakdown?.xgboost?.trend || "NEUTRAL"}</strong>
                    <span className={(result.signal_breakdown?.xgboost?.trend === 'BUY' || result.signal_breakdown?.xgboost?.trend === 'BULLISH') ? "text-green-500 font-bold ml-1.5" : (result.signal_breakdown?.xgboost?.trend === 'SELL' || result.signal_breakdown?.xgboost?.trend === 'BEARISH') ? "text-red-500 font-bold ml-1.5" : "text-text-muted font-bold ml-1.5"}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-text-muted opacity-50 border-2 border-dashed border-border rounded-2xl min-h-[500px]">
