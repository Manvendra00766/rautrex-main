    ...
    <div className="flex flex-col gap-6 max-w-7xl mx-auto w-full pb-12">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          {results && (
            <Button onClick={handleExport} variant="outline" className="bg-card border-border gap-2">
      ...
      <CardSurface className="overflow-hidden">
        ...
        >
          <span className="text-sm font-bold text-text-muted flex items-center gap-2 uppercase tracking-wider">
              ...
              <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    <Label className="text-xs text-text-muted uppercase font-bold">Path Sample Size</Label>
                ...
                <div className="space-y-4">
                   <Label className="text-xs text-text-muted uppercase font-bold">Projection Horizon</Label>
                   <Tabs value={horizon} onValueChange={setHorizon} className="w-full">
                    <TabsList className="grid grid-cols-5 bg-card border border-border h-10">
                ...
                <div className="space-y-4">
                  <Label className="text-xs text-text-muted uppercase font-bold">Capital Commitment</Label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-accent font-bold font-mono">$</span>
                    <Input type="number" value={investment} onChange={e => setInvestment(Number(e.target.value))} className="pl-8 bg-card border-border font-mono text-lg" />
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        {loading ? (
          <div className="flex flex-col gap-6">
             <div className="grid grid-cols-4 gap-4 h-32">
                {[1,2,3,4].map(i => <CardSurface key={i} className="animate-pulse bg-muted/50" />)}
             </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <MetricCard 
                  ...
                  subtext={`${(((results.expected_value || investment)/investment - 1)*100).toFixed(1)}% Return`}
                  trend={((results.expected_value || investment)/investment - 1) >= 0 ? "up" : "down"}
                />
                <MetricCard 
                  title="Value at Risk (95%)" 
                  value={`$${(results.var || 0).toLocaleString(undefined, {maximumFractionDigits: 0})}`} 
                  subtext="Max Probable Loss"
                />
                <MetricCard 
                  title="Prob. of Profit" 
                  value={`${(results.prob_profit || 0).toFixed(1)}%`} 
                  subtext="Likelihood of Upside"
                />
                <MetricCard 
                  title="Annual Volatility" 
                  value={`${((results.volatility || 0) * 100).toFixed(1)}%`} 
                  subtext="Historical Std Dev"
                />
            </div>

            {/* Main Area Chart */}
            <CardSurface className="p-6 h-[500px] relative overflow-hidden">
...
<ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData}>
                       ...
                       />
                       {/* Percentile Bands */}
                       <Area type="monotone" dataKey="p95" stroke="none" fill="var(--accent-purple)" fillOpacity={0.05} />
                       <Area type="monotone" dataKey="p75" stroke="none" fill="var(--accent-purple)" fillOpacity={0.1} />
                       <Area type="monotone" dataKey="p25" stroke="none" fill="var(--accent-purple)" fillOpacity={0.1} />
                       <Area type="monotone" dataKey="p5" stroke="none" fill="var(--accent-purple)" fillOpacity={0.05} />
                       {/* Median Path */}
                       <Line type="monotone" dataKey="p50" name="Median Price" stroke="var(--accent-teal)" strokeWidth={3} dot={false} activeDot={{ r: 6, fill: '#fff', stroke: 'var(--accent-teal)', strokeWidth: 2 }} />
                    </AreaChart>
                  </ResponsiveContainer>
</ChartWrapper>
               </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
               <CardSurface className="p-6">
                  <SectionHeader 
                    ...
                    className="mb-6 uppercase text-sm tracking-wider"
                  />
...
<ResponsiveContainer width="100%" height="100%">
               ...
               <CardSurface className="p-6 flex flex-col justify-center gap-6">
                  <SectionHeader 
                  ...
                  <div className="space-y-4">
                     <p className="text-xs text-text-muted leading-relaxed">
                        Based on {sims[0]} simulated price paths, the expected portfolio value at the end of {horizon} days is <span className="text-text-primary font-bold">${results.expected_value.toLocaleString()}</span>. 
                     </p>
                     <div className="p-4 rounded-xl bg-accent/5 border border-accent/10">
                        <p className="text-xs text-accent font-bold mb-1 uppercase tracking-widest">Risk Summary</p>
                        <p className="text-xs text-text-muted">There is a <span className="text-text-primary font-bold">{results.prob_profit.toFixed(1)}%</span> probability that the asset will trade above your initial investment of ${investment.toLocaleString()}.</p>
                     </div>
                  </div>
               </CardSurface>
            </div>
