                  ...
                  <div className="space-y-2 max-h-[250px] overflow-auto pr-2 custom-scrollbar">
                    {portfolio.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-3 p-3 rounded-xl bg-card border border-border">
                        <div className="w-10 h-10 rounded-lg bg-muted/50 flex items-center justify-center font-bold text-xs text-text-primary">{item.ticker}</div>
                        <div className="flex-1">
                          <div className="h-1.5 w-full bg-muted/50 rounded-full overflow-hidden">
                            <div className="h-full bg-accent" style={{ width: `${item.weight * 100}%` }} />
                          </div>
                        </div>
                        <div className="w-16 text-right font-mono text-xs font-bold text-text-primary">{(item.weight * 100).toFixed(1)}%</div>
                        <button onClick={() => removeAsset(idx)} className="p-1 hover:text-red-500 text-text-muted transition-colors"><X size={14} /></button>
                      </div>
                    ))}
                  </div>

                  <div className="grid grid-cols-5 gap-2 pt-2 border-t border-border">
                     <Input placeholder="Ticker" value={newTicker} onChange={e => setNewTicker(e.target.value)} className="col-span-2 bg-card h-9 text-xs" />
                     <Input placeholder="Weight (e.g. 0.2)" type="number" value={newWeight} onChange={e => setNewWeight(e.target.value)} className="col-span-2 bg-card h-9 text-xs" />
          ...
          <div className="flex flex-col gap-8">
             ...
             <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <MetricCard 
                  ...
                  subtext="Aggregate Risk (0-100)"
                  trend={(results.risk_score || 0) > 70 ? "down" : "up"}
                />
                <MetricCard 
                  title="Max Drawdown" 
                  value={`${((results.metrics?.max_drawdown || 0) * 100).toFixed(2)}%`} 
                  icon={<TrendingDown className="text-red-500" />} 
                  subtext={`Recovery: ${results.metrics?.max_dd_duration_days || 0} days`}
                  trend="down"
                />
                <MetricCard 
                  title="Sharpe Ratio" 
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <CardSurface className="lg:col-span-2 p-6 h-[450px]" style={{ minHeight: '450px' }}>
                  <SectionHeader 
                    title="HISTORICAL DRAWDOWN CURVE" 
...
<ResponsiveContainer width="100%" height="100%" aspect={2.5}>
                   ...
                   <div className="flex-1 grid grid-cols-3 grid-rows-3 gap-1">
                      {results.correlation_matrix.map((item: any, i: number) => (
                        <div key={i} className="group relative flex items-center justify-center rounded-md text-xs font-mono font-bold" 
                           ...
                           {/* Hover Ticker Info */}
                           <div className="absolute -top-10 scale-0 group-hover:scale-100 bg-card border border-border px-2 py-1 rounded text-text-primary pointer-events-none transition-all z-20 whitespace-nowrap">
                             {item.x} vs {item.y}
                           </div>
                        </div>
                      ))}
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-12">
                ...
                <CardSurface className="p-6 flex flex-col h-full">
                  <SectionHeader 
